...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public boolean insert_recipe(int rec_id, String rec_name, String rec_desc, String rec_link){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
...
public ArrayList<String> getAllCatNames()
    ...
    {
        Cursor cursor = getReadableDatabase().rawQuery("select * from categories", null);
    ...
    {
        Cursor cursor = getReadableDatabase().rawQuery("select * from ingredients where ing_id = 25", null);
        return cursor;
    }
}
